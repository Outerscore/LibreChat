...
class AgentClient extends BaseClient {
  ...
  async buildMessages(messages, parentMessageId, _buildOptions, opts) {
    ...
    /**
     * Outerscore: the invisible per-request canvas document (sent via promptPrefix)
     * folded into the PRIMARY agent's dynamic system tail only, so a selected
     * compliance agent audits the actual document without it appearing in the
     * visible chat. Sub-agents never receive it.
     */
    ...
    await Promise.all(
      allAgents.map(({ agent, agentId }) => {
        ...
        if (outerscoreContext && agentId === this.options.agent.id) {
          agentRunContextParts.push(outerscoreContext);
        }
